...
from factorlab.factors.simple import LiquidityShockFactor, MomentumFactor, SizeFactor, VolatilityFactor
...
def _merge_registry(
    ...
    for name, ctor in incoming.items():
        if name in out:
            msg = f"Factor '{name}' already registered; override by plugin source: {source}"
            if on_error == "raise":
