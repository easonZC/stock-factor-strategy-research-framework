...
def _build_effective_factor_definitions(
    factor_names: list[str],
    factor_registry: dict[str, Any],
...
def _resolve_strategy_definition(
    ...
    if mode == "sign":
        return StrategyDefinition(
            ...
            implementation="factorlab.workflows.config_runner._build_sign_weights",
            origin="builtin",
        )
    return strategy_definition_from_instance(_build_strategy(back_cfg, strategy_registry=strategy_registry))
...
def run_from_config(
    ...
    data_lineage = describe_panel_lineage(
        panel=panel,
        data_cfg=data_cfg,
        load_report=load_report,
        mode_report=mode_report,
    ...
    effective_factor_definitions = _build_effective_factor_definitions(
    ...
    factor_definition_outputs = write_factor_definition_artifacts(
        out_dir=out,
        definitions=effective_factor_definitions,
    )
    ...
    if back_cfg["enabled"]:
        ...
        effective_strategy_definition = _resolve_strategy_definition(
            back_cfg=back_cfg,
            strategy_registry=strategy_registry_for_run,
        )
        strategy_definition_outputs = write_strategy_definition_artifacts(
            out_dir=out,
            definitions=[effective_strategy_definition],
        )
    ...
    runtime_manifest = collect_runtime_manifest(repo_root=repo_root)
    ...
    experiment_registry_path = write_json_artifact(out / "experiment_registry.json", experiment_registry)
