...
class TopKLongStrategy(Strategy):
    """等权 TopK 多头策略。"""
    STRATEGY_FAMILY = "long_only"
    STRATEGY_DESCRIPTION = "Long-only top-K strategy with optional cap and rebalance cadence."
    STRATEGY_CONSTRAINTS = ("long_only", "gross<=1", "optional_max_weight")
    STRATEGY_TAGS = ("equity", "ranking", "baseline")
...
class LongShortQuantileStrategy(Strategy):
    """做多高分位、做空低分位的等绝对权重策略。"""
    STRATEGY_FAMILY = "long_short"
    STRATEGY_DESCRIPTION = "Dollar-neutral long-short strategy using symmetric top/bottom quantile baskets."
    STRATEGY_CONSTRAINTS = ("dollar_neutral", "symmetric_quantiles", "optional_max_weight")
    STRATEGY_TAGS = ("equity", "ranking", "market_neutral")
...
class FlexibleLongShortStrategy(Strategy):
    ...
    STRATEGY_FAMILY = "flexible_long_short"
    STRATEGY_DESCRIPTION = "Configurable long-short allocator supporting long-only mode and asymmetric legs."
    STRATEGY_CONSTRAINTS = ("optional_long_only", "configurable_leg_sizes", "optional_max_weight")
    STRATEGY_TAGS = ("equity", "allocation", "flexible")
