            ...
            for variant in variants:
                ...
                fac_fig_paths.append(
                    register_figure(
                        ...
                        variant=variant,
                        label="IC Decay",
                        source_tables=[decay_csv],
                        description="IC decay figure generated from same-run IC decay tables.",
                    )
                )
                ...
                if not fmb_summary.empty:
                    fac_fig_paths.append(
                        register_figure(
                            ...
                            factor=fac,
                            variant=variant,
                            label="Fama-MacBeth Beta",
                            source_tables=[fmb_sum_csv, fmb_coef_csv],
                            description="Fama-MacBeth beta chart generated from same-run regression tables.",
                        )
                    )
                    ...
                    if not ind_summary.empty:
                        fac_fig_paths.append(
                            register_figure(
                                ...
                                factor=fac,
                                variant=variant,
                                label="Industry Decomposition",
                                source_tables=[ind_sum_csv, ind_detail_csv],
                                description="Industry decomposition chart generated from same-run grouped return tables.",
                            )
                        )
                    ...
                    if not style_summary.empty:
                        fac_fig_paths.append(
                            register_figure(
                                ...
                                factor=fac,
                                variant=variant,
                                label="Style Decomposition",
                                source_tables=[style_sum_csv, style_detail_csv],
                                description="Style decomposition chart generated from same-run grouped return tables.",
                            )
                        )
                ...
                fac_fig_paths.extend(
                    [
                        register_figure(
                            ...
                            variant=variant,
                            label="IC Series",
                            source_tables=[primary_ic_csv] if primary_ic_csv is not None else [],
                            description="Primary IC / RankIC series generated from same-run IC tables.",
                        ),
                        register_figure(
                            plot_quantile_nav(
                                q_nav,
                            ...
                            variant=variant,
                            label="Quantile NAV",
                            source_tables=[q_nav_csv, q_profile_csv, q_daily_csv],
                            description="Quantile NAV chart generated from same-run quantile return tables.",
                        ),
                        register_figure(
                            plot_turnover(
                                q_turn,
                            ...
                            variant=variant,
                            label="Turnover",
                            source_tables=[q_turn_csv],
                            description="Turnover chart generated from same-run quantile turnover tables.",
                        ),
                        register_figure(
                            plot_coverage(
                                cov,
                            ...
                            variant=variant,
                            label="Coverage",
                            source_tables=[cov_csv],
                            description="Coverage chart generated from same-run factor coverage tables.",
                        ),
                        register_figure(
                            plot_stability(
                                st,
                            ...
                            variant=variant,
                            label="Stability",
                            source_tables=[st_csv],
                            description="Stability chart generated from same-run factor stability tables.",
                        ),
                    ]
                )
        ...
        if not outlier_df.empty:
            ...
            figure_map.setdefault("global", []).append(
                register_figure(
                    ...
                    factor=None,
                    variant="global",
                    label="Outlier Monitoring",
                    source_tables=[outlier_csv],
                    description="Global outlier monitoring chart generated from same-run preprocessing audit tables.",
                    scope="global",
                )
            )
            ...
            if not corr_s.empty:
                ...
                figure_map.setdefault("global", []).append(
                    register_figure(
                        plot_corr_heatmap(
                            corr_s,
                            assets_dir / "factor_corr_spearman.png",
                            title="Factor Spearman Correlation",
                        ),
                        factor=None,
                        variant="global",
                        label="Factor Correlation",
                        source_tables=[corr_s_csv, corr_p_csv],
                        description="Global factor correlation heatmap generated from same-run correlation tables.",
                        scope="global",
                    )
                )
