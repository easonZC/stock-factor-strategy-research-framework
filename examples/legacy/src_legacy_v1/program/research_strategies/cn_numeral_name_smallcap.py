"""
Research strategy: keep names containing Chinese numerals, then select smallest market-cap stocks.
"""

import pandas as pd

name = file_name = __file__.replace("\\", "/").split("/")[-1].replace(".py", "")
period_offset = ["3_0", "3_1", "3_2"]
factors = {"总市值": []}
select_count = 10


def screen_universe(data: pd.DataFrame) -> pd.DataFrame:
    """Apply tradability filters and keep names containing Chinese numerals."""
    ...
    regex = "|".join(numerals)
    data = data[data["股票名称"].str.contains(regex) == True]
    return data


def rank_and_pick(data: pd.DataFrame, count: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Rank by ascending market cap and keep top-N."""
