...
- Factor research (primary):
  ...
  - TS-specific signal lag profile (lag-IC decay table + plot)
...
```
Detailed operator semantics are documented in `docs/config_overrides.md`.
...
- CLI `run_from_config` supports repeated `--config` deep-merge and repeated `--set key.path=value` overrides.
  - `--set` operators:
  - `=` replace value
  - `+=` append/merge (list append/extend, dict deep-merge)
  - `-=` remove (list item removal, dict key removal)
- CLI `run_from_config` pre-validates config schema by default; use `--skip-schema-validation` to bypass.
- CLI `run_from_config --validate-only` performs schema validation and exits (no data load/research run).
